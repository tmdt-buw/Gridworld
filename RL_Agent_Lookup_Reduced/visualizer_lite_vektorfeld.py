...
import numpy as np
import json
from RL_Agent_Lookup_Reduced.Agent import q_values_aus_lookup
...
def toggle_qvals(agent):
    global show_qvals, lookup
    lookup = agent
    show_qvals = True

def visualize(states, pause):
    grid_states = []
    positions = []
    deaths = []
    state_mines = []
    ...
    for state, pos, dead in zip(grid_states, positions, deaths):
        ...
        for mine in state_mines[sm]:
            ax.text(mine[0], 9-mine[1], 'M', va='center', ha='center', fontsize=25, color='w')
        for wall in state_walls[sw]:
            ax.text(wall[0], 9-wall[1], 'W', va='center', ha='center', fontsize=25, color='w')
        for qvals in state_qvals[sq]:
            if qvals[3] == 0:
                ax.arrow(qvals[0], 9-qvals[1], 0.0, 0.5, length_includes_head=True,
          ...
          head_width=0.2, head_length=0.2)
                ...
                ax.arrow(qvals[0], 9-qvals[1], -0.5, 0.0, length_includes_head=True,
          head_width=0.2, head_length=0.2)
        ...
        if dead:
            ax.text(pos[0], 9-pos[1], 'T', va='center', ha='center', fontsize=25, color='w')
        else:
            ax.text(pos[0], 9-pos[1], 'S', va='center', ha='center', fontsize=15, color='w')
